CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."SAT_PGLIB_users_INIT"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- SAT_TGT
	...
	TRUNCATE TABLE "LIBPG_FL"."SAT_PGLIB_users"  CASCADE;
	...
	( 
		SELECT 
			  "STG_INR_SRC"."users_HKEY" AS "users_HKEY"
			, "STG_INR_SRC"."LOAD_DATE" AS "LOAD_DATE"
			, "STG_INR_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM(  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."first_name"),'~'),'#','\' || 
				'#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."last_name"),'~'),'#','\' || '#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."email"),'~'),'#','\' || '#'))|| '#' ,'#' || '~'),'~') ,'SHA1'),'HEX')) AS "HASH_DIFF"
			, 'N'::text AS "DELETE_FLAG"
			, "STG_INR_SRC"."last_updated" AS "last_updated"
			, "STG_INR_SRC"."user_id" AS "user_id"
			, "STG_INR_SRC"."last_name" AS "last_name"
			, "STG_INR_SRC"."first_name" AS "first_name"
			, "STG_INR_SRC"."email" AS "email"
			, ROW_NUMBER()OVER(PARTITION BY "STG_INR_SRC"."users_HKEY" ORDER BY "STG_INR_SRC"."LOAD_DATE","STG_INR_SRC"."LOAD_CYCLE_ID") AS "DUMMY"
		FROM "PostgresLibrary_STG"."users" "STG_INR_SRC"
	)
	SELECT 
		  "STG_SRC"."users_HKEY" AS "users_HKEY"
		, "STG_SRC"."LOAD_DATE" AS "LOAD_DATE"
		, "STG_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
		, "STG_SRC"."HASH_DIFF" AS "HASH_DIFF"
		, "STG_SRC"."DELETE_FLAG" AS "DELETE_FLAG"
		, "STG_SRC"."last_updated" AS "last_updated"
		, "STG_SRC"."user_id" AS "user_id"
