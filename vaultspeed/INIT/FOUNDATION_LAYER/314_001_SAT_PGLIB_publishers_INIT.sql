CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."SAT_PGLIB_publishers_INIT"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- SAT_TGT
	...
	TRUNCATE TABLE "LIBPG_FL"."SAT_PGLIB_publishers"  CASCADE;
	...
	( 
		SELECT 
			  "STG_INR_SRC"."publishers_HKEY" AS "publishers_HKEY"
			, "STG_INR_SRC"."LOAD_DATE" AS "LOAD_DATE"
			, "STG_INR_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM(  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."name"),'~'),'#','\' || '#')
				)|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."address"),'~'),'#','\' || '#'))|| '#' ,'#' || '~'),'~') ,'SHA1'),'HEX')) AS "HASH_DIFF"
			...
			, "STG_INR_SRC"."address" AS "address"
			, ROW_NUMBER()OVER(PARTITION BY "STG_INR_SRC"."publishers_HKEY" ORDER BY "STG_INR_SRC"."LOAD_DATE",
				"STG_INR_SRC"."LOAD_CYCLE_ID") AS "DUMMY"
		FROM "PostgresLibrary_STG"."publishers" "STG_INR_SRC"
	)
	SELECT 
		  "STG_SRC"."publishers_HKEY" AS "publishers_HKEY"
