CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."EXT_PGLIB_checkouts_INIT"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- EXT_TGT
	...
	TRUNCATE TABLE "PostgresLibrary_EXT"."checkouts"  CASCADE;

	INSERT INTO "PostgresLibrary_EXT"."checkouts"(
		 "LOAD_CYCLE_ID"
		,"LOAD_DATE"
		,"last_updated"
		,"__$operation"
	...
	( 
		SELECT 
			...
			, COALESCE("INI_SRC"."checkout_id", CAST("MEX_INR_SRC"."KEY_ATTRIBUTE_INTEGER" AS INTEGER)) AS "checkout_id"
		...
		FROM "lib_mgmt"."checkouts" "INI_SRC"
	...
	( 
		SELECT 
			...
			, "LOAD_INIT_DATA"."checkout_id" AS "checkout_id"
		...
		SELECT 
			...
			, CAST("MEX_EXT_SRC"."KEY_ATTRIBUTE_INTEGER" AS INTEGER) AS "checkout_id"
	...
	( 
		SELECT 
			...
			, "PREP_EXCEP"."checkout_id" AS "checkout_id"
			...
			, UPPER( "PREP_EXCEP"."book_id"::text) AS "book_id_FK_bookid_BK"
	...
	SELECT 
		...
		, "CALCULATE_BK"."checkout_id" AS "checkout_id"
