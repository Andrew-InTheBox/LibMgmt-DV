CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."BV_PIT_daily-snapshot_authors_PIT_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- PIT_UPD
	...
	( 
		...
		FROM "LIBPG_BV"."VIEW_PIT_daily-snapshot_authors_SNAPSHOTDATES" "SSDV_SRC_UPD"
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("SAT_ED_SRC_UPD1"."LOAD_DATE")OVER(PARTITION BY "SAT_ED_SRC_UPD1"."authors_HKEY" ORDER BY "SAT_ED_SRC_UPD1"."LOAD_DATE")
	...
	( 
		SELECT 
			  UPPER(ENCODE(DIGEST( "HUB_SRC_UPD"."authors_HKEY"::text || '#' || TO_CHAR("SNAPSHOTDATES_UPD"."SNAPSHOT_TIMESTAMP",
				'DD/MM/YYYY HH24:MI:SS.US'::varchar) ,'SHA1'),'HEX')) AS "PIT_daily-snapshot_authors_HKEY"
			...
			, COALESCE("SAT_SRC_UPD1"."LOAD_DATE","UNSAT_SRC_UPD1"."LOAD_DATE") AS "SAT_PGLIB_authors_LOAD_DATE"
	...
	( 
		SELECT 
			  "MIV_UPD"."PIT_daily-snapshot_authors_HKEY" AS "PIT_daily-snapshot_authors_HKEY"
			...
			, "MIV_UPD"."SAT_PGLIB_authors_LOAD_DATE" AS "SAT_PGLIB_authors_LOAD_DATE"
		FROM "MIV_UPD" "MIV_UPD"
		LEFT OUTER JOIN "LIBPG_BV"."PIT_daily-snapshot_authors" "PIT_SRC_UPD" ON  "MIV_UPD"."PIT_daily-snapshot_authors_HKEY" = "PIT_SRC_UPD"."PIT_daily-snapshot_authors_HKEY" AND "MIV_UPD"."SAT_PGLIB_authors_HKEY" =
			"PIT_SRC_UPD"."SAT_PGLIB_authors_HKEY" AND "MIV_UPD"."SAT_PGLIB_authors_LOAD_DATE" = "PIT_SRC_UPD"."SAT_PGLIB_authors_LOAD_DATE"
		WHERE  "PIT_SRC_UPD"."PIT_daily-snapshot_authors_HKEY" IS NULL
	)
	UPDATE "LIBPG_BV"."PIT_daily-snapshot_authors" "PIT_UPD_TGT"
	SET 
		 "SAT_PGLIB_authors_HKEY" =  "UPD_DATA_SRC"."SAT_PGLIB_authors_HKEY"
		,"SAT_PGLIB_authors_LOAD_DATE" =  "UPD_DATA_SRC"."SAT_PGLIB_authors_LOAD_DATE"
	FROM  "UPD_DATA_SRC"
	WHERE "PIT_UPD_TGT"."PIT_daily-snapshot_authors_HKEY" =  "UPD_DATA_SRC"."PIT_daily-snapshot_authors_HKEY"
	;
END;
...
BEGIN -- PIT_TGT
	...
	INSERT INTO "LIBPG_BV"."PIT_daily-snapshot_authors"(
		 "PIT_daily-snapshot_authors_HKEY"
		,"authors_HKEY"
		,"SNAPSHOT_TIMESTAMP"
		,"LOAD_CYCLE_ID"
		,"SAT_PGLIB_authors_HKEY"
		,"SAT_PGLIB_authors_LOAD_DATE"
	)
	WITH "SNAPSHOTDATES" AS 
	( 
		SELECT 
			  "SSDV_SRC"."SNAPSHOT_TIMESTAMP" AS "SNAPSHOT_TIMESTAMP"
		FROM "LIBPG_BV"."VIEW_PIT_daily-snapshot_authors_SNAPSHOTDATES" "SSDV_SRC"
	...
	( 
		SELECT 
			...
			, COALESCE(LEAD("SAT_ED_SRC1"."LOAD_DATE")OVER(PARTITION BY "SAT_ED_SRC1"."authors_HKEY" ORDER BY "SAT_ED_SRC1"."LOAD_DATE")
				, TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS'::varchar)) AS "LOAD_END_DATE"
	...
	( 
		SELECT 
			...
			, COALESCE("SAT_SRC1"."LOAD_DATE","UNSAT_SRC1"."LOAD_DATE") AS "SAT_PGLIB_authors_LOAD_DATE"
	...
	( 
		SELECT 
			  "MIV"."PIT_daily-snapshot_authors_HKEY" AS "PIT_daily-snapshot_authors_HKEY"
			...
			, "MIV"."SAT_PGLIB_authors_LOAD_DATE" AS "SAT_PGLIB_authors_LOAD_DATE"
		FROM "MIV" "MIV"
		LEFT OUTER JOIN "LIBPG_BV"."PIT_daily-snapshot_authors" "PIT_SRC" ON  "MIV"."PIT_daily-snapshot_authors_HKEY" = "PIT_SRC"."PIT_daily-snapshot_authors_HKEY" AND "MIV"."SAT_PGLIB_authors_HKEY" =
			"PIT_SRC"."SAT_PGLIB_authors_HKEY" AND "MIV"."SAT_PGLIB_authors_LOAD_DATE" = "PIT_SRC"."SAT_PGLIB_authors_LOAD_DATE"
		WHERE  "PIT_SRC"."PIT_daily-snapshot_authors_HKEY" IS NULL
	)
	SELECT 
		  "DATA_SRC"."PIT_daily-snapshot_authors_HKEY" AS "PIT_daily-snapshot_authors_HKEY"
		...
		, "DATA_SRC"."SAT_PGLIB_authors_LOAD_DATE" AS "SAT_PGLIB_authors_LOAD_DATE"
