CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."SET_FMC_MTD_FL_INCR_PGLIB"(p_dag_name varchar,
...
BEGIN -- HIST_INS
	...
	( 
		...
		WHERE  "FMCH_SRC"."SRC_BK" = 'PGLIB' AND "FMCH_SRC"."success_flag" = 1 AND "FMCH_SRC"."LOAD_CYCLE_ID" < p_LOAD_CYCLE_ID::int
	)
	SELECT 
...
BEGIN -- LWT_INS
	...
	TRUNCATE TABLE "PostgresLibrary_MTD"."FMC_LOADING_WINDOW_TABLE"  CASCADE;

	INSERT INTO "PostgresLibrary_MTD"."FMC_LOADING_WINDOW_TABLE"(
		 "FMC_BEGIN_LW_TIMESTAMP"
		,"FMC_END_LW_TIMESTAMP"
	)
	SELECT 
	...
	FROM "LibraryMgmt_FMC"."FMC_LOADING_HISTORY" "FMCOH_SRC"
	WHERE  "FMCOH_SRC"."LOAD_CYCLE_ID" = p_LOAD_CYCLE_ID::int
	;
END;
