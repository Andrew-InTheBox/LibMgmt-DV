CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."EXT_PGLIB_authors_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- EXT_TGT
	...
	( 
		SELECT 
			...
			, COALESCE(UPPER( "TDFV_SRC"."author_id"::text),"MEX_SRC"."KEY_ATTRIBUTE_INTEGER") AS "author_id_BK"
			, "TDFV_SRC"."first_name" AS "first_name"
			, "TDFV_SRC"."last_name" AS "last_name"
		FROM "PostgresLibrary_DFV"."VW_authors" "TDFV_SRC"
	...
	( 
		SELECT 
			...
			, CURRENT_TIMESTAMP + row_number() over (PARTITION BY  "CALCULATE_BK"."author_id_BK"  ORDER BY  "CALCULATE_BK"."last_updated")
	...
	SELECT 
		...
		, "EXT_UNION"."author_id_BK" AS "author_id_BK"
