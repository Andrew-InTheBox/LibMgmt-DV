CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."HUB_PGLIB_publishers_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- HUB_TGT
	...
	INSERT INTO "LIBPG_FL"."HUB_publishers"(
		 "publishers_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"publisher_id_BK"
	...
	( 
		SELECT 
			...
			, 0 AS "LOGPOSITION"
			, "STG_SRC1"."publisher_id_BK" AS "publisher_id_BK"
			, "STG_SRC1"."SRC_BK" AS "SRC_BK"
			, 0 AS "GENERAL_ORDER"
		FROM "PostgresLibrary_STG"."publishers" "STG_SRC1"
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."publisher_id_BK" AS "publisher_id_BK"
			, "CHANGE_SET"."SRC_BK" AS "SRC_BK"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."publishers_HKEY" ORDER BY "CHANGE_SET"."GENERAL_ORDER",
	...
	SELECT 
		...
		, "MIN_LOAD_TIME"."publisher_id_BK" AS "publisher_id_BK"
		, "MIN_LOAD_TIME"."SRC_BK" AS "SRC_BK"
	FROM "MIN_LOAD_TIME" "MIN_LOAD_TIME"
	LEFT OUTER JOIN "LIBPG_FL"."HUB_publishers" "HUB_SRC" ON  "MIN_LOAD_TIME"."publishers_HKEY" = "HUB_SRC"."publishers_HKEY"
	WHERE  "MIN_LOAD_TIME"."DUMMY" = 1 AND "HUB_SRC"."publishers_HKEY" is NULL
	;
END;
