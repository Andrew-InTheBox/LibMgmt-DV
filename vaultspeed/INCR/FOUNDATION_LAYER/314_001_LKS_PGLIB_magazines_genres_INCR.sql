CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."LKS_PGLIB_magazines_genres_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- LKS_TEMP_TGT
	...
	TRUNCATE TABLE "PostgresLibrary_STG"."LKS_PGLIB_magazines_genres_TMP"  CASCADE;

	INSERT INTO "PostgresLibrary_STG"."LKS_PGLIB_magazines_genres_TMP"(
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_magazines_genres_HKEY" AS "LNK_magazines_genres_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_magazines_genres_HKEY" AS "LNK_magazines_genres_HKEY"
		...
		FROM "LIBPG_FL"."LKS_PGLIB_magazines_genres" "LKS_SRC"
		INNER JOIN "LIBPG_FL"."LNK_magazines_genres" "LNK_SRC" ON  "LKS_SRC"."LNK_magazines_genres_HKEY" = "LNK_SRC"."LNK_magazines_genres_HKEY"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "LNK_SRC"."magazines_HKEY" = "DIST_STG"."magazines_HKEY"
	)
	SELECT 
		  "TEMP_TABLE_SET"."LNK_magazines_genres_HKEY" AS "LNK_magazines_genres_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND "TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."genres_HKEY"::text =
			LAG("TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."genres_HKEY"::text,1)OVER(PARTITION BY "TEMP_TABLE_SET"."magazines_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
...
BEGIN -- LKS_INUR_TGT
	...
	INSERT INTO "LIBPG_FL"."LKS_PGLIB_magazines_genres"(
		 "LNK_magazines_genres_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"DELETE_FLAG"
	...
	SELECT 
		  "LKS_TEMP_SRC_INUR"."LNK_magazines_genres_HKEY" AS "LNK_magazines_genres_HKEY"
	...
	FROM "PostgresLibrary_STG"."LKS_PGLIB_magazines_genres_TMP" "LKS_TEMP_SRC_INUR"
