CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."LKS_PGLIB_books_publishers_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- LKS_TEMP_TGT
	...
	TRUNCATE TABLE "PostgresLibrary_STG"."LKS_PGLIB_books_publishers_TMP"  CASCADE;

	INSERT INTO "PostgresLibrary_STG"."LKS_PGLIB_books_publishers_TMP"(
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_books_publishers_HKEY" AS "LNK_books_publishers_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_books_publishers_HKEY" AS "LNK_books_publishers_HKEY"
		...
		FROM "LIBPG_FL"."LKS_PGLIB_books_publishers" "LKS_SRC"
		INNER JOIN "LIBPG_FL"."LNK_books_publishers" "LNK_SRC" ON  "LKS_SRC"."LNK_books_publishers_HKEY" = "LNK_SRC"."LNK_books_publishers_HKEY"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "LNK_SRC"."books_HKEY" = "DIST_STG"."books_HKEY"
	)
	SELECT 
		  "TEMP_TABLE_SET"."LNK_books_publishers_HKEY" AS "LNK_books_publishers_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND "TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."publishers_HKEY"::text =
			LAG("TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."publishers_HKEY"::text,1)OVER(PARTITION BY "TEMP_TABLE_SET"."books_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
...
BEGIN -- LKS_INUR_TGT
	...
	INSERT INTO "LIBPG_FL"."LKS_PGLIB_books_publishers"(
		 "LNK_books_publishers_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"DELETE_FLAG"
	...
	SELECT 
		  "LKS_TEMP_SRC_INUR"."LNK_books_publishers_HKEY" AS "LNK_books_publishers_HKEY"
	...
	FROM "PostgresLibrary_STG"."LKS_PGLIB_books_publishers_TMP" "LKS_TEMP_SRC_INUR"
