CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."SAT_PGLIB_authors_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- SAT_TEMP_TGT
	...
	TRUNCATE TABLE "PostgresLibrary_STG"."SAT_PGLIB_authors_TMP"  CASCADE;

	INSERT INTO "PostgresLibrary_STG"."SAT_PGLIB_authors_TMP"(
		 "authors_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"RECORD_TYPE"
	...
	( 
		SELECT DISTINCT 
 			  "STG_DIS_SRC"."authors_HKEY" AS "authors_HKEY"
			, "STG_DIS_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
		FROM "PostgresLibrary_STG"."authors" "STG_DIS_SRC"
	...
	( 
		SELECT 
			...
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM(  UPPER(REPLACE(COALESCE(TRIM( "STG_TEMP_SRC"."first_name"),'~'),'#','\' || 
				'#'))|| '#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_TEMP_SRC"."last_name"),'~'),'#','\' || '#'))|| '#' ,'#' || '~'),'~') ,'SHA1'),'HEX')) AS "HASH_DIFF"
		...
		FROM "PostgresLibrary_STG"."authors" "STG_TEMP_SRC"
		WHERE  "STG_TEMP_SRC"."RECORD_TYPE" = 'S'
		UNION ALL 
		SELECT 
			  "SAT_SRC"."authors_HKEY" AS "authors_HKEY"
			, "SAT_SRC"."LOAD_DATE" AS "LOAD_DATE"
			, "SAT_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, MAX("SAT_SRC"."LOAD_DATE")OVER( PARTITION BY "SAT_SRC"."authors_HKEY") AS "LOAD_END_DATE"
			, 'SAT' AS "RECORD_TYPE"
			, 'SAT' AS "SOURCE"
			, 0 AS "ORIGIN_ID"
			...
			, "SAT_SRC"."author_id" AS "author_id"
			, "SAT_SRC"."first_name" AS "first_name"
			, "SAT_SRC"."last_name" AS "last_name"
		FROM "LIBPG_FL"."SAT_PGLIB_authors" "SAT_SRC"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "SAT_SRC"."authors_HKEY" = "DIST_STG"."authors_HKEY"
	)
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND "TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."HASH_DIFF"::text =
			LAG("TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."HASH_DIFF"::text,1)OVER(PARTITION BY "TEMP_TABLE_SET"."authors_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
...
BEGIN -- SAT_INUR_TGT
	...
	SELECT 
		  "SAT_TEMP_SRC_INUR"."authors_HKEY" AS "authors_HKEY"
		...
		, "SAT_TEMP_SRC_INUR"."author_id" AS "author_id"
		, "SAT_TEMP_SRC_INUR"."first_name" AS "first_name"
		, "SAT_TEMP_SRC_INUR"."last_name" AS "last_name"
	FROM "PostgresLibrary_STG"."SAT_PGLIB_authors_TMP" "SAT_TEMP_SRC_INUR"
