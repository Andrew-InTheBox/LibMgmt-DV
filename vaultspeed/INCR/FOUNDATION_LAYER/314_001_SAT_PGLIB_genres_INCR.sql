CREATE OR REPLACE FUNCTION "LibraryMgmt_PROC"."SAT_PGLIB_genres_INCR"() 
RETURNS void 
LANGUAGE 'plpgsql' 
...
BEGIN -- SAT_TEMP_TGT
	...
	TRUNCATE TABLE "PostgresLibrary_STG"."SAT_PGLIB_genres_TMP"  CASCADE;

	INSERT INTO "PostgresLibrary_STG"."SAT_PGLIB_genres_TMP"(
		 "genres_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"RECORD_TYPE"
	...
	( 
		SELECT DISTINCT 
 			  "STG_DIS_SRC"."genres_HKEY" AS "genres_HKEY"
			, "STG_DIS_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
		FROM "PostgresLibrary_STG"."genres" "STG_DIS_SRC"
	...
	( 
		SELECT 
			...
			, 'STG' AS "SOURCE"
			, 1 AS "ORIGIN_ID"
			, UPPER(ENCODE(DIGEST(COALESCE(RTRIM(  UPPER(REPLACE(COALESCE(TRIM( "STG_TEMP_SRC"."genre_name"),'~'),'#','\' || 
			...
			, "STG_TEMP_SRC"."genre_name" AS "genre_name"
		FROM "PostgresLibrary_STG"."genres" "STG_TEMP_SRC"
		WHERE  "STG_TEMP_SRC"."RECORD_TYPE" = 'S'
		UNION ALL 
		SELECT 
			  "SAT_SRC"."genres_HKEY" AS "genres_HKEY"
			, "SAT_SRC"."LOAD_DATE" AS "LOAD_DATE"
			, "SAT_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, MAX("SAT_SRC"."LOAD_DATE")OVER( PARTITION BY "SAT_SRC"."genres_HKEY") AS "LOAD_END_DATE"
			, 'SAT' AS "RECORD_TYPE"
			, 'SAT' AS "SOURCE"
			, 0 AS "ORIGIN_ID"
			...
			, "SAT_SRC"."genre_id" AS "genre_id"
			, "SAT_SRC"."genre_name" AS "genre_name"
		FROM "LIBPG_FL"."SAT_PGLIB_genres" "SAT_SRC"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "SAT_SRC"."genres_HKEY" = "DIST_STG"."genres_HKEY"
	)
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND "TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."HASH_DIFF"::text =
			LAG("TEMP_TABLE_SET"."DELETE_FLAG"::text || "TEMP_TABLE_SET"."HASH_DIFF"::text,1)OVER(PARTITION BY "TEMP_TABLE_SET"."genres_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
		...
		, "TEMP_TABLE_SET"."genre_name" AS "genre_name"
...
BEGIN -- SAT_INUR_TGT
	...
	SELECT 
		  "SAT_TEMP_SRC_INUR"."genres_HKEY" AS "genres_HKEY"
		...
		, "SAT_TEMP_SRC_INUR"."genre_id" AS "genre_id"
		, "SAT_TEMP_SRC_INUR"."genre_name" AS "genre_name"
	FROM "PostgresLibrary_STG"."SAT_PGLIB_genres_TMP" "SAT_TEMP_SRC_INUR"
