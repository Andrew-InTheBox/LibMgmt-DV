...
Vaultspeed version: 6.0.0.5, generation date: 2026/03/17 17:43:38
...
if (path_to_mtd / "179_mappings_lib_pg_init_20260317_174338.json").exists():
	with open(path_to_mtd / "179_mappings_lib_pg_init_20260317_174338.json") as file: 
		mappings = json.load(file)

else:
	with open(path_to_mtd / "mappings_lib_pg_init.json") as file: 
		mappings = json.load(file)

lib_pg_init = DAG(
	dag_id="lib_pg_init", 
	default_args=default_args,
	description="lib_pg_init", 
...
fmc_mtd = JdbcOperator(
	...
	sql=f"""select "LibraryMgmt_PROC"."SET_FMC_MTD_FL_INIT_PGLIB"(p_dag_name := '{{{{ dag_run.dag_id }}}}', p_LOAD_CYCLE_ID := '{{{{ dag_run.id }}}}', p_LOAD_DATE := '{{{{ execution_date.strftime(\"%Y-%m-%d %H:%M:%S.%f\") }}}}');""", 
	dag=lib_pg_init
)
...
# Analyze tables
if (path_to_mtd / "179_FL_mtd_lib_pg_init_20260317_174338.json").exists():
	with open(path_to_mtd / "179_FL_mtd_lib_pg_init_20260317_174338.json") as file: 
		analyze_data = json.load(file)
else:
	with open(path_to_mtd / "FL_mtd_lib_pg_init.json") as file: 
		analyze_data = json.load(file)
